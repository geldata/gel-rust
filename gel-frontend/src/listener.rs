...
use crate::{
    ...
    stream_type::{
        ...
        StreamState, StreamType, UnknownStreamType, ALPN_EDGEDB_BINARY, ALPN_GEL_BINARY,
        ALPN_HTTP1_1, ALPN_HTTP2, ALPN_POSTGRESQL, STR_EDGEDB_BINARY, STR_HTTP1_1, STR_HTTP2,
        STR_POSTGRESQL,
    },
};
use futures::StreamExt;
use gel_auth::postgres::server::ConnectionEvent;
use gel_stream::{ResolvedTarget, TlsAlpn};
...
async fn handle_connection_inner(
    ...
    match stream_type {
        StreamType::GelBinary => handle_stream_gel_binary(socket, identity, bound_config).await,
...
async fn handle_stream_gel_binary(
    mut socket: ListenerStream,
    identity: ConnectionIdentityBuilder,
    bound_config: impl IsBoundConfig,
) -> Result<(), std::io::Error> {
        ...
        ssl.set_servername_callback(move |ssl, alert| {
            ...
            let ssl_new = ssl_new.maybe_configure(move |ctx| {
                ctx.alpn = config
                    ...
                    .set_alpn_select_callback(move |_, alpn| {
                        trace!(
...
fn bind_task<C: ListenerConfig>(
    ...
    async move {
        ...
        while let Some(addresses) = stm.next().await.transpose()? {
            ...
            let new_listeners = HashSet::from_iter(addresses);
